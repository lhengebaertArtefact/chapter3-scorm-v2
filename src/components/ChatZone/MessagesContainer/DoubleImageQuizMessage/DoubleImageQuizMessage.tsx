...
import { DoubleImageQuizObjective, Objective, QuizObjective } from "../../../../types/Objective";
...
const DoubleImageQuizMessage = ({ objective, container }: { objective: DoubleImageQuizObjective, container: HTMLDivElement | null }) => {
    ...
    <>
      <div className={`h-fit w-4/5 my-2 relative ${isDisplayed ? '' : 'hidden'}`}>
        ...
        <div className="flex flex-col justify-start items-start my-7 bg-bordersCyan/30 px-2">
          ...
          <div
            className="w-full h-fit flex flex-row justify-between items-start my-4"
          >
            {
                objective.content.images.map((img,id)=>(
                    <img 
                        className="w-[45%]"
                        src={img} 
                        key={id} 
...
export default DoubleImageQuizMessage;